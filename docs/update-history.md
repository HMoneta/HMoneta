...
| 📅 日期           | 🎯 版本           | 🏷️ 主题      | 🔑 核心特性                                  |
|-----------------|-----------------|-------------|------------------------------------------|
| **2026年4月6日**   | V0.0.2-Alpha1   | 功能增强        | UniFi网络管理、雷池WAF站点功能、前端UI优化           |
| **2026年3月30日**  | V0.0.2-Alpha1   | 构建升级        | Maven构建系统 (从Gradle迁移)                      |
| **2026年2月16日**  | V0.0.1          | 发布Release版本 |                                          |
| **2026年1月10日**  | V0.0.1-Beta2    | 架构升级        | Gradle现代化、构建优化、前端配置升级                    |
...
- **UnifiController**: UniFi管理API入口，支持本地和远程两种授权方式
- **UnifiApiService**: UniFi API服务，使用 Spring WebFlux WebClient 实现响应式 HTTP 客户端
- **WebApiUtil**: 通用 Web API 工具类，使用 Builder 模式构建可配置的 WebClient
...
- **Maven 构建系统**: 从 Gradle 迁移到 Maven 构建系统，删除 Gradle 相关文件，添加 pom.xml
...
- **证书申请优化**: 证书30天内到期时变更为申请证书而非下载
- **证书信息获取**: 增加获取指定域名证书的 crt、key 文本功能
- **错误处理增强**: 增加证书不存在错误类型
- **异步连接问题**: 修复因为异步导致的无法正常获取 Unifi 连接情况
- **空指针异常**: 修复可能出现的空指针异常问题
- **连接性检查**: 修复检查 Unifi 可连接性的逻辑问题
...
- **统一配置**: 使用`build.gradle`统一管理依赖和构建脚本
- **前端配置升级**: 前端使用ES模块格式的Vite配置文件，提升构建性能

> **📝 后续更新**: V0.0.2-Alpha1 已从 Gradle 迁移到 Maven 构建系统

#### 🔧 技术栈优化

- **Spring Boot升级**: 升级到Spring Boot 4.0.1，保持与最新生态兼容
- **现代化工具**: 使用最新的构建工具和开发环境
- **性能提升**: 通过现代化构建系统获得更好的性能表现
- **开发体验**: 改善开发环境的配置和构建体验

#### 📊 构建系统改进

- **Gradle Wrapper**: 使用Gradle Wrapper确保构建环境一致性
- **构建缓存**: 启用Gradle Configuration Cache优化构建性能
- **依赖管理**: 改进依赖管理和版本控制
- **多环境支持**: 优化不同环境下的构建配置
