...
- **配置管理**: Spring Dotenv
- **日志系统**: SLF4J + Logback + MDC日志追踪
- **API**: RESTful API
- **工具库**: Apache Commons Text, Commons Validator, Bouncy Castle, Hypersistence Utils for Hibernate, Jackson (JSON processing)
...
19. **雷池 WAF 集成**: 支持雷池 WAF API 令牌管理、SSL证书列表查询、证书添加、BaseUrl设置
20. **雷池 WAF SSL 证书管理**: 新增 `LeiChiWafSslService` 服务类，封装 SSL 证书相关 API 调用功能
21. **雷池 WAF AOP 切面**: 为需访问雷池 API 的方法增加统一切面，提供日志记录和异常处理支持
22. **UniFi Network 集成**: 支持 UniFi Network API 集成，可管理网络站点和客户端信息
23. **UniFi 本地/远程授权**: 支持 UniFi 本地和远程 API 授权方式
24. **网络管理页面**: 新增 network.vue 页面，展示 UniFi 站点和客户端信息
25. **网络服务预留**: 新增 NetworkService 网络服务模块，为后续功能扩展预留
...
# 或者运行打包后的 JAR
java -jar target/HMoneta-0.0.2-Alpha1.jar
```
...
- `HMfront/hm-front/`: Vue 3 前端项目
  - `src/pages/`: 页面组件 (about.vue, dns.vue, index.vue, login.vue, loginNew.vue（全新重写）, logPage.vue, network.vue, pluginManager.vue, setting.vue, HelloWorld.vue)
  - `src/components/`: 通用组件
...
- `POST /hm/unifi/setting`: 设置 UniFi API 连接信息（支持本地和远程两种授权方式）
- `GET /hm/unifi/info`: 获取当前 UniFi 设置信息
- `GET /hm/unifi/status`: 检查 UniFi 连接状态
- `GET /hm/unifi/sites/info`: 获取站点列表信息
- `GET /hm/unifi/sites/clients/info`: 获取站点客户端信息

### 雷池 WAF 管理 API

- `POST /hm/waf/lc/token`: 设置雷池 WAF API 令牌
- `GET /hm/waf/lc/info`: 获取雷池 WAF 设置信息
- `POST /hm/waf/lc/baseUrl`: 设置雷池 WAF 基础URL
- `GET /hm/waf/lc/ssl/list`: 获取雷池 WAF SSL 证书列表
- `POST /hm/waf/lc/ssl/modify`: 向雷池 WAF 添加 SSL 证书
...
### 雷池 WAF 集成
- 支持与雷池 WAF 系统集成进行 Web 应用防火墙管理和 SSL 证书监控
- LeiChiSettingEntity: 雷池 WAF 设置实体，存储 API 连接信息（baseUrl、token）
- LeiChiWafApiResp: 雷池 API 通用响应结构，支持泛型 data 字段和列表数据包装（ListData）
- LeiChiSslInfoResp: 雷池 SSL 证书信息响应，包含证书列表（nodes）和总数（total）
- 系统自动管理设置的唯一性，保存新设置时会删除旧设置

### UniFi Network 集成
- 使用 Spring WebFlux 和 Netty 实现响应式 HTTP 客户端
- 使用 Builder 模式构建可配置的 WebClient
- 支持 SSL 证书验证跳过（用于本地 API）
- 支持本地和远程两种 API 授权方式
- 提供了完整的功能：主机列表查询、本地站点列表查询、连接客户端列表查询
...
- 增加了对DNS更新失败情况的处理机制
- UniFi API 客户端使用 Netty HTTP 客户端，优化连接管理
